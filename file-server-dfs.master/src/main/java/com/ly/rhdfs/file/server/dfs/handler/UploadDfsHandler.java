    ...
    public Mono<ServerResponse> uploadFileMasterRequest(ServerRequest request) {
        return authenticationVerify.verifyAuthentication(request)
                .flatMap(resultInfo -> {
                    ...
                    long fileSize= ConvertUtil.parseLong(request.queryParam(serverConfig.getFileSizeParamName()).orElse("0"),0);
                    ...
                    return tokenFactory.createUploadToken(path,fileName)
                            .flatMap(tokenInfo -> {
