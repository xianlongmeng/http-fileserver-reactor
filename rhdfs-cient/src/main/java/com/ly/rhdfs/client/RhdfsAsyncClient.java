...
public class RhdfsAsyncClient {
    ...
    public Mono<Void> uploadFileAsync(String localFileName, String baseUrl, String path, String fileName,
        ...
        return WebClient.create(baseUrl).get()
                .uri(UriComponentsBuilder.fromUriString("/dfs/upload-request/{path}").queryParam("fileName", fileName)
                        .queryParam("fileSize", fileSize).toUriString(), path)
                .accept(MediaType.APPLICATION_JSON).retrieve().bodyToFlux(TokenFileInfo.class)
                ...
                .then(DataBufferUtils
                        ...
                        .flatMap(etag -> WebClient.create(baseUrl).get()
                ...
                fileChannel.read(byteBuffer, startPos, byteBuffer, new CompletionHandler<Integer, ByteBuffer>() {
                    ...
                    public void completed(Integer result, ByteBuffer attachment) {
                        ...
                        WebClient
                                ...
                                .retryWhen(Retry.fixedDelay(10, Duration.ofSeconds(30))).subscribe(clientResponse -> {
                            if (clientResponse.statusCode().is2xxSuccessful())
                                sink.success();
                            else
    ...
    public Mono<Void> receiveFileChunkPieceAsync(FileChunkPieceTransfer fileChunkPieceTransfer, File file, int chunkPieceSize) {
        ...
        return Mono
                .create(sink -> {
                    ...
                    int index = fileChunkPieceTransfer.getTimes() / fileChunkPieceTransfer.getTimes();
                ...
                .retryWhen(Retry.fixedDelay(10 * fileChunkPieceTransfer.getFileInfo().getFileChunkList().get(fileChunkPieceTransfer.getChunk()).getChunkServerIdList().size(), Duration.ofSeconds(30)))
                .then();
    }
}
